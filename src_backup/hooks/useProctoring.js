...
export const useProctoring = (config, quizId) => {
    ...
    const setupTabMonitoring = useCallback(() => {
        if (!config.enable_tab_monitoring)
            return;
    ...
    const setupCopyPasteBlocking = useCallback(() => {
        if (!config.enable_copy_paste_block)
            return;
        const blockEvent = (e) => {
            e.preventDefault();
    ...
    const saveProctoringData = useCallback(async (disconnectReason) => {
